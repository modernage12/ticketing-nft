...
const handleListTicket = async (tokenId, originalPriceString) => {
  ...
  try {
    ...
    listSuccessMessage.value = `Biglietto ${tokenId} messo in vendita! Tx Hashes: ${approveHash}... / ${listHash}...`;
...
// Funzione per annullare l'offerta
 const handleCancelListing = async (tokenId) => {
    ...
    try {
        ...
        cancelSuccessMessage.value = `Offerta per Ticket ID ${tokenId} annullata! Hash: ${cancelHash}...`;
        // Lo store ricarica myTickets e listings dopo successo
    } catch (error) {
...
<style scoped>
 ...
 li.ticket-card { border: 1px solid #555; padding: 0.75rem; margin-bottom: 0.75rem; background-color: #2a2a2a; border-radius: 4px; font-size: 0.9em; display: flex; justify-content: space-between; align-items: center;}
 hr { margin: 1rem 0; }
